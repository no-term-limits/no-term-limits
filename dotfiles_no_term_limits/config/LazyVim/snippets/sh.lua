function debug_logger(message)
  vim.notify(message, "info", { timeout = 60000 })
end
...
return {
  ...
  --   "u",
  --   f(function(_, snip)
  --     -- local regexForVariableAssignments = '^(w+)=?"?%${?(d)(:-w*)?}?"?'
  --     -- let regexForVariableAssignments = '\v^(\w+)\="?\$\{?(\d)(:-\w*)?\}?"?'
  --     local regexForVariableAssignments = '^(%w+)="?${(%d)'
  --     local argVariableAssignmentLineNumbers = {}
  --     for lineNumber, line in ipairs(vim.api.nvim_buf_get_lines(0, 0, -1, false)) do
  --       -- print(lineNumber)
  --       -- print("HEY")
  --       -- print(line)
  --       local match, match1 = string.match(line, regexForVariableAssignments)
  --       debug_logger(match)
  --       -- debug_logger(match1)
  --       if match then
  --         table.insert(argVariableAssignmentLineNumbers, lineNumber + 1)
  --       end
  --     end
  --
