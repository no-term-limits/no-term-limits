...
availability=$(run_headless_nvim "$temp_home" +"e $file" +'lua vim.bo.filetype="python"; local conform=require("conform"); local names=conform.list_formatters_for_buffer(0); print("DECLARED=" .. table.concat(names, ",")); local formatter=conform.get_formatter_info("ruff_format", 0); print("AVAILABLE=" .. tostring(formatter.available)); print("AVAILABLE_MSG=" .. tostring(formatter.available_msg))' +"qall!")
...
run_headless_nvim "$temp_home" +"e $file" +'lua vim.bo.filetype="python"; require("conform").format({ async = false, lsp_format = "never" })' +"wqall!"
